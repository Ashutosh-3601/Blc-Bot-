...
module.exports.run = async (bot, message, args) => {
	...
	const badwords = args.join('porn', 'pornhub', 'big tits', 'black dick', 'black cock', 'pussy', 'hentai', 'nsfw', 'ecchi');
	if (!badwords) {return message.channel.send(':underage: This is not an nsfw channel :underage:');}
		...
		const result = (await Promise.all(
			$.querySelectorAll('.r')
				...
				.map(async (e) => {
					...
					try {
						obj.title = new parse(body.text).querySelector('head').childNodes.find(e.tagName === 'title').childNodes[0].text;
					}
					catch (err) {
		...
		if (!result.length) return searchmessage.edit(`${body.lang.noRslt}` + term);
		const first = result.shift();
		const vanityurl_1 = /^https?:\/\/[\\.\-_]+(?::\d+|\.\w*)(?:\/|$)/g.exec(first.url);
		...
		const embeds = result.reduce((acc, r) => {
			const vu_1 = /^https?:\/\/[\\.\-_]+(?::\d+|\.\w*)(?:\/|$)/g.exec(r.url);
...
/* module.exports.run = async (client, message, args) => {
	...
	const searchMessage = await message.reply('Searching... Sec.');
	const se = args.join(' ');
	const searchUrl = `https://www.google.com/search?q=${encodeURIComponent(se)}`;
